      ...
      <div className="min-h-screen bg-zinc-950 flex flex-col">
        ...
        <main className="flex-1 flex items-center justify-center py-16 px-4">
          <div className="max-w-2xl w-full">
            <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-sm text-center" data-testid="assessment-result">
              <CheckCircle className="w-16 h-16 text-green-500 mx-auto mb-6" />
              <h1 className="text-3xl font-semibold tracking-tight text-zinc-50 mb-4" style={{fontFamily: 'Outfit, sans-serif'}}>
                Assessment Submitted
              </h1>
              <p className="text-zinc-400 mb-8">Your assessment has been submitted successfully.</p>
              
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                <div className="bg-zinc-950 border border-zinc-800 p-4 rounded-sm">
                  <div className="text-2xl font-semibold text-zinc-50 mb-1" style={{fontFamily: 'Outfit, sans-serif'}} data-testid="result-score">
                    {result.score}%
                  </div>
                  <div className="text-sm text-zinc-400">Score</div>
                </div>
                <div className="bg-zinc-950 border border-zinc-800 p-4 rounded-sm">
                  <div className="text-2xl font-semibold text-zinc-50 mb-1" style={{fontFamily: 'Outfit, sans-serif'}} data-testid="result-correct">
                    {result.correct}
                  </div>
                  <div className="text-sm text-zinc-400">Correct</div>
                </div>
                <div className="bg-zinc-950 border border-zinc-800 p-4 rounded-sm">
                  <div className="text-2xl font-semibold text-zinc-50 mb-1" style={{fontFamily: 'Outfit, sans-serif'}} data-testid="result-total">
                    {result.total}
                  </div>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-sm mb-6">
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div>
                <h1 className="text-2xl font-semibold tracking-tight text-zinc-50 mb-2" style={{fontFamily: 'Outfit, sans-serif'}} data-testid="assessment-title">
                  {assessment?.title}
                </h1>
                <p className="text-sm text-zinc-400">{assessment?.description}</p>
              </div>
              <div className="flex items-center gap-2 bg-zinc-950 border border-zinc-800 px-4 py-3 rounded-sm" data-testid="timer">
                <Clock className={`w-5 h-5 ${timeLeft < 300 ? 'text-red-500' : 'text-blue-500'}`} />
                <span className={`text-xl font-mono font-semibold ${timeLeft < 300 ? 'text-red-500' : 'text-zinc-50'}`}>
                  {formatTime(timeLeft)}
                </span>
              </div>
            </div>
          ...
          <div className="space-y-6 mb-6">
            {assessment?.questions.map((q, qIndex) => (
              <div key={qIndex} className="bg-zinc-900 border border-zinc-800 p-6 rounded-sm" data-testid={`question-${qIndex}`}>
                <div className="flex gap-3 mb-4">
                  ...
                  <p className="text-zinc-100 flex-1">{q.question}</p>
                </div>
                  ...
                  {q.options.map((option, oIndex) => (
                    ...
                    >
                      <input
                        ...
                        className="mt-1 w-4 h-4 text-blue-600 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-zinc-900"
                      />
